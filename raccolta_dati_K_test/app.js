...
// === SPOT CONDITIONS LOGIC ===
  
  let spotConditionsInitialized = false;
  ...
  const SPOT_MOCK_DATA = {
    "punta trettu": {
      ...
      rider: {
        rider_has_data: true,
        rider_reports_count: 3,
        rider_wind_knots_min: 16,
        rider_wind_knots_max: 22,
        rider_last_update: "2026-04-21T17:45:00Z"
      },
      anemometer: {
        anemometer_has_data: true,
        anemometer_wind_knots_avg: 19,
        anemometer_wind_knots_gust: 24,
        anemometer_last_update: "2026-04-21T18:25:00Z"
      },
      forecast: {
        forecast_has_data: true,
        forecast_wind_knots_min: 18,
        forecast_wind_knots_max: 25,
        forecast_valid_to: "2026-04-21T22:00:00Z",
        forecast_trend_code: "steady"
      }
    },
    "stagnone": {
      spot_id: "spot_002",
      status_label: "Attivo",
      status_color: "#2dd4bf",
      updated_at: "2026-04-21T18:15:00Z",
      rider: {
        rider_has_data: false,
        rider_reports_count: 0,
        rider_wind_knots_min: 0,
        rider_wind_knots_max: 0,
        rider_last_update: null
      },
      anemometer: {
        anemometer_has_data: false,
        anemometer_wind_knots_avg: 0,
        anemometer_wind_knots_gust: 0,
        anemometer_last_update: null
      },
      forecast: {
        forecast_has_data: true,
        forecast_wind_knots_min: 14,
        forecast_wind_knots_max: 20,
        forecast_valid_to: "2026-04-21T22:00:00Z",
        forecast_trend_code: "up"
      }
    },
    "porto pollo": {
      spot_id: "spot_003",
      ...
      rider: {
        rider_has_data: true,
        rider_reports_count: 1,
        rider_wind_knots_min: 14,
        rider_wind_knots_max: 18,
        rider_last_update: "2026-04-21T16:30:00Z"
      },
      anemometer: {
        anemometer_has_data: true,
        anemometer_wind_knots_avg: 16,
        anemometer_wind_knots_gust: 20,
        anemometer_last_update: "2026-04-21T17:40:00Z"
      },
      forecast: {
        forecast_has_data: false,
        forecast_wind_knots_min: 0,
        forecast_wind_knots_max: 0,
        forecast_valid_to: null,
        forecast_trend_code: null
      }
    }
  };

  function initSpotConditions() {
    if (spotConditionsInitialized) return;
    spotConditionsInitialized = true;
    
    const locationField = document.getElementById("location");
    let debounceTimer;
    
    locationField.addEventListener("input", function(e) {
      clearTimeout(debounceTimer);
      const spotInput = e.target.value.trim();
      
      debounceTimer = setTimeout(() => {
        if (spotInput.length >= 3) {
          updateSpotConditions(spotInput);
        } else {
          showSpotPlaceholder();
        }
      }, 300);
  ...
  function updateSpotConditions(spotInput) {
    const normalizedInput = normalizeSpotInput(spotInput);
    const matchedSpot = matchSpot(normalizedInput);
    
    if (matchedSpot) {
      const mockData = getMockSpotData(matchedSpot);
      renderSpotData(mockData);
    } else {
      showSpotNoData();
    }
  }
  ...
  function matchSpot(normalizedInput) {
    const knownSpots = Object.keys(SPOT_MOCK_DATA);
    const spotAliases = {
      "stagnone": ["lo stagnone", "laghetto"],
      "punta trettu": ["trettu", "punta tretu"],
      "porto pollo": ["porto polo"]
    };
    
    // 1. Exact match
    if (knownSpots.includes(normalizedInput)) {
      return normalizedInput;
    }
    
    // 2. Alias match
    for (const [canonical, aliases] of Object.entries(spotAliases)) {
      if (aliases.includes(normalizedInput)) {
        return canonical;
      }
    }
    
    // 3. Partial match solo se univoco
    const partialMatches = knownSpots.filter(spot => 
      spot.includes(normalizedInput) || normalizedInput.includes(spot)
    );
  ...
  function getMockSpotData(spotKey) {
    return SPOT_MOCK_DATA[spotKey] || null;
  }

  function isEstimatedData(data) {
    return !data.rider.rider_has_data && 
           !data.anemometer.anemometer_has_data && 
           data.forecast.forecast_has_data;
  }
  ...
  function renderSpotData(data) {
    ...
    // Status generale
    document.getElementById("spotStatus").textContent = data.status_label;
    document.getElementById("spotUpdated").textContent = formatUpdateTime(data.updated_at);
    
    // Contenitore principale
    const spotContainer = document.getElementById("spotData");
    
    // Stato stimato
    if (isEstimatedData(data)) {
      spotContainer.classList.add("spot-forecast-only");
      document.getElementById("spotConditionsSubtitle").textContent = t("spot_estimated_data");
    } else {
      spotContainer.classList.remove("spot-forecast-only");
    }
    
    // Render aree
    renderRiderData(data.rider);
    renderAnemometerData(data.anemometer);
    renderForecastData(data.forecast);
  }

  function renderRiderData(rider) {
    const container = document.getElementById("riderData");
    
    if (!rider.rider_has_data) {
      container.innerHTML = `<span class="spot-no-data">${t("spot_no_rider_reports")}</span>`;
      return;
    }
    
    const windRange = `${rider.rider_wind_knots_min}-${rider.rider_wind_knots_max} ${t("spot_wind_unit")}`;
    const reports = rider.rider_reports_count === 1 
      ? `1 ${t("spot_reports_singular")}` 
      : `${rider.rider_reports_count} ${t("spot_reports_plural")}`;
    
    container.innerHTML = `
      <div class="spot-wind-range">${windRange}</div>
      <div>${reports}</div>
      <div class="spot-updated-text">${formatUpdateTime(rider.rider_last_update)}</div>
    `;
  }

  function renderAnemometerData(anemometer) {
    const container = document.getElementById("anemometerData");
    
    if (!anemometer.anemometer_has_data) {
      container.innerHTML = `<span class="spot-no-data">${t("spot_no_anemometer_data")}</span>`;
      return;
    }
    
    const windInfo = anemometer.anemometer_wind_knots_gust > anemometer.anemometer_wind_knots_avg 
      ? `${anemometer.anemometer_wind_knots_avg} ${t("spot_wind_unit")} (rafiche ${anemometer.anemometer_wind_knots_gust})`
      : `${anemometer.anemometer_wind_knots_avg} ${t("spot_wind_unit")}`;
    
    container.innerHTML = `
      <div class="spot-wind-range">${windInfo}</div>
      <div class="spot-updated-text">${formatUpdateTime(anemometer.anemometer_last_update)}</div>
    `;
  }
  ...
  function renderForecastData(forecast) {
    ...
    if (!forecast.forecast_has_data) {
      container.innerHTML = `<span class="spot-no-data">${t("spot_no_forecast_data")}</span>`;
      return;
    }
    
    const windRange = `${forecast.forecast_wind_knots_min}-${forecast.forecast_wind_knots_max} ${t("spot_wind_unit")}`;
    const trend = getTrendEmoji(forecast.forecast_trend_code);
    const validUntil = `${t("spot_valid_until")} ${formatTime(forecast.forecast_valid_to)}`;
    
    container.innerHTML = `
      <div class="spot-wind-range">${trend} ${windRange}</div>
      <div>${validUntil}</div>
    `;
  }

  function showSpotPlaceholder() {
    document.getElementById("spotPlaceholder").hidden = false;
    document.getElementById("spotData").hidden = true;
    document.getElementById("spotConditionsSubtitle").textContent = t("spot_conditions_subtitle");
  }

  function showSpotNoData() {
    document.getElementById("spotPlaceholder").hidden = false;
    document.getElementById("spotData").hidden = true;
    document.getElementById("spotConditionsSubtitle").textContent = t("spot_no_data");
  }

  function hidePlaceholder() {
    document.getElementById("spotPlaceholder").hidden = true;
  }

  function showSpotData() {
    document.getElementById("spotData").hidden = false;
  }
  ...
  function formatUpdateTime(timestamp) {
    ...
    if (diffMins < 60) {
      return `${diffMins} ${t("spot_minutes_ago")}`;
    } else if (diffMins < 1440) {
      return `${Math.floor(diffMins / 60)} ${t("spot_hours_ago")}`;
    } else {
  ...
  function getTrendEmoji(trendCode) {
    const trends = {
      'up': '📈',
      'down': '📉', 
      'steady': '➡️'
    };
    return trends[trendCode] || '➡️';
  }
