...
async def geocode_candidates(name: str, *, timeout_s: float, count: int = 10) -> list[dict[str, Any]]:
    ...
    for item, query_variant, language, source_rank in raw_results:
        ...
        ql = query.lower()
        name_l = name_value.lower()
        variant_l = str(query_variant).lower()

        candidate_type = "other"
        candidate_notes = []

        if name_l == ql:
            candidate_type = "direct_exact"
            candidate_notes.append("exact_name_match")

        elif name_l == variant_l and variant_l != ql:
            candidate_type = "translated_alias"
            candidate_notes.append("matched_via_alias")

        elif variant_l != ql and name_l != variant_l:
            candidate_type = "weak_related"
            candidate_notes.append("provider_returned_different_name")

        elif ql in name_l or name_l in ql:
            candidate_type = "partial_match"
            candidate_notes.append("partial_string_overlap")
